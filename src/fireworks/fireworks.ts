import {FireworkBody, ParticleSystem} from "./particle_system";
...
function draw() {
    ...
    if (firework_timer > firework_max_timer) {
        ...
        for (let i = 0; i < count; i++) {
            particleSystem.particles.push(new FireworkBody(
                particleSystem,
                new Vec2(random_range(-200, 200) + width / 2, height),
                new Vec2(random_range(-250, 250), random_range(-1000, -1300))));
        }
    }
