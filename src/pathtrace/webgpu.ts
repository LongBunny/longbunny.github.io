...
// @ts-ignore
import compute_shader from './shaders/raytrace.wgsl?raw';
// @ts-ignore
import render_shader from './shaders/screen.wgsl?raw';
...
export async function startWebGpuRenderer(
    ...
    // shaders
    const compute = await load_compute_shader();
    const render = await load_render_shader();
...
async function load_compute_shader(): Promise<string> {
    return compute_shader;
}

async function load_render_shader(): Promise<string> {
